...
<form id="formulario" name="f1" id="f1">

</form>
<div style="border:2px solid gren;width:80%;">
<h3 align="center">Consulta por servicios</h3>


<form id="proveedoresL" action="?id=listado" method="post">
 <div id="slect" style="border:2px solid rd;margin-left:50px;width:70px;">
 <select class="browser-default" name="queryocupacion">
<?
$result= $conexion->query("select DISTINCT servicio from proveedores");
  while($rw=$result->fetch_array()){
  ?>  
      <option><?echo $rw['servicio']?></option>
    <?
    }
?>
...
<input type="submit" id="consulta" class="btn btn-large" value="Consultar">

</form>
<form id="formubic">

<h3 align="center">Consulta por Zona</h3>
<span class="etiquetas">Que estado de Venezuela deseas consultar?</span>
</br>
<?adresconsulta('estados','estados')?>
